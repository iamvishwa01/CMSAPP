...
<body>
  ...
  <div class="container">
    <section>
      <h2>Add User</h2>
      <div class="row">
        <form class="col s12" method="POST" action="adduser.php">
          <div class="row">
                ...
                <div class="row">
                  <div class="input-field col s3">
                    <select name="userroles">
                      <?php
                      $roles = "select user_role from roles order by user_role desc ";
                      $roles_result = mysqli_query($conn, $roles);
                      while ($roless = mysqli_fetch_assoc($roles_result)) {
                        $user_role = $roless['user_role'];
                        echo "<option value='" . $roless['user_role'] . "'>" . $roless['user_role'] . "</option>";
                      }
  ...
  if (isset($_POST['submit'])) {
    ...
    $sqlcheck_username = "select * from users where username='$uname' ";
    $sqlcheck_email = "select * from users where email='$email' ";
    ...
    if (empty($fname) && empty($lname) && empty($uname) && empty($email) && empty($password) && empty($mobileno)) {
      ...
      echo "<script>Materialize.toast('Username already present!!', 4000, 'red darken-2 rounded');</script>";
    } else if (mysqli_num_rows($res_email) > 0) {
      echo "<script>Materialize.toast('Email already present!!', 4000, 'red darken-2 rounded');</script>";
    } else {
      trim($fname);
      trim($lname);
      trim($uname);
      ...
      $sql = "insert into users (fname,lname,username,email,password,mobile_no,user_role,Joined_date ) values ('$fname','$lname','$uname','$email','$hashed_password','$mobileno','$user_roles', now() ) ";
